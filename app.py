...
def _inject_native_pivot_table(xlsx_bytes, df_columns, table_name, pivot_sheet_name, row_field, data_field, data_field_label, col_field=None, subtotal="count", data_field_2=None, data_field_label_2=None, subtotal_2="count"):
    ...
    data_idx_2 = df_columns.index(data_field_2) if data_field_2 else None
    ...
    for i in range(n_fields):
        ...
        elif i == data_idx or i == data_idx_2:
...
def build_excel_with_native_pivot(result_df: pd.DataFrame, data_sheet_name: str, key_prefix: str):
    ...
    final_bytes = _inject_native_pivot_table(
        ...
        subtotal="sum",
        data_field_2=account_col,
        data_field_label_2="Count of Customer Account Number",
        subtotal_2="count",
    )
...
def render_pivot_section(df: pd.DataFrame, key_prefix: str):
    ...
    الصفوف = المحصل، القيم = مجموع (SUM) عمود التصنيف وCount لعمود Customer Account Number."""
    ...
    with st.expander("📊 معاينة الـ Pivot Table (هتلاقي النسخة الحقيقية القابلة للتعديل جوه ملف الإكسيل بعد التحميل)", expanded=False):
        ...
        pivot_df = pivot_df.rename_axis(index="المحصل").rename(columns={class_col: "مجموع التصنيف", account_col: "Count of Customer Account Number"})
