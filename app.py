...
def _inject_native_pivot_table(
    ...
    data_fields_xml = (
        f'<dataFields count="{len(data_fields_parts)}">'
...
def _render_period_results(stored, period_key):
    """عرض النتائج المحفوظة للفترة: جدول + رسوم + Pivot + تنزيل ملف Excel."""
    ...
    if result_df is not None:
        excel_bytes, pivot_added = build_excel_with_native_pivot(
            result_df, "النتائج", f"period_{period_key}"
        )
        if not pivot_added:
            st.caption(
                "⚠️ اتنزل الملف من غير Pivot Table لأن عمود المحصل (Collected by) "
                "أو رقم حساب العميل (Customer Account number) مش موجود في الملف."
            )
        st.download_button(
