...
def build_dashboard_html(df, class_col, sales_col, time_col, source_name="", filter_hint="", filter_summary=None,
    ...
    parts = [
        ...
        ".charts-grid-2{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin:0 0 12px;align-items:stretch}",".charts-grid-2 > .chart-card{display:flex;flex-direction:column;min-height:100%}",".charts-grid-2 .js-plotly-plot,.charts-grid-2 .plotly-graph-div{width:100% !important}",
...
def _build_wallet_analysis(df):
    ...
    if float(work[WALLET_COLLECTED_COL].sum()) > 0 or float(work[WALLET_REMAINING_COL].sum()) > 0:
        ...
        _wallet_bar_style(
            ...
            height=400, xaxis_title="عمر الإسناد", yaxis_title="المبلغ",
            show_legend=True,
