...
# لوحة ألوان هادئة موحّدة لتويب النشاط + تصدير HTML
ACTIVITY_AGENT_PALETTE = [
    "#3D7A7C", "#4F8C8D", "#5F9A9B", "#6FA3A4", "#82B0B1",
    "#95BDBE", "#A8C9CA", "#72A0A8", "#8AADB4",
]
ACTIVITY_STATE_PALETTE = ["#3D7A7C", "#5F9A9B", "#82B0B1", "#A8C9CA"]
ACTIVITY_OUTCOME_COLORS = {"ناجحة": "#3D8B7A", "غير ناجحة": "#8A9BA8"}
ACTIVITY_EXPORT = {
    "bg": "#F3F6F8",
    "surface": "#FFFFFF",
    "border": "#E2E8F0",
...
def _activity_layout(**overrides):
    """تنسيق موحّد هادئ لكل رسوم داشبورد النشاط (Streamlit + HTML)."""
    grid = THEME.get("border_soft", "rgba(148,163,184,0.12)")
    base = {
        **PLOTLY_LAYOUT,
        "title_x": 0.02,
        "title_xanchor": "left",
        "title_font": dict(size=15, color=THEME["text"], family="Tajawal, Tahoma, sans-serif"),
        "font": dict(size=12, color=THEME["text_dim"], family="Tajawal, Tahoma, sans-serif"),
        "height": 360,
        "bargap": 0.22,
        "legend_title_text": "",
        "legend": {
        ...
        "margin": dict(t=48, b=78, l=52, r=24),
        "xaxis": dict(showgrid=False, automargin=True, title_font=dict(size=12), tickfont=dict(size=11)),
        "yaxis": dict(
            showgrid=True, gridcolor=grid, zeroline=False, automargin=True,
...
def _format_wasted_display(minutes):
    """عرض الوقت المهدر بشكل مقروء."""
    try:
        minutes = float(minutes or 0)
    except (TypeError, ValueError):
        minutes = 0.0
    if minutes >= 120:
        return f"{minutes / 60:,.1f} ساعة"
    return f"{minutes:,.0f} دقيقة"


def _style_activity_bars(fig, horizontal=False):
    fig.update_traces(
        marker_line_width=0,
        textposition="none",
        cliponaxis=False,
        hovertemplate=(
            "<b>%{y}</b><br>%{fullData.name}: %{x:,}<extra></extra>"
            if horizontal else
...
def render_activity_kpi_cards(total, success, agent_count, success_rate, wasted_minutes):
    """كروت مؤشرات هادئة بواجهة Streamlit الأصلية."""
    fail = max(int(total) - int(success), 0)
    with st.container(border=True):
        c1, c2, c3, c4, c5 = st.columns(5)
        c1.metric("عدد المحصّلين", f"{int(agent_count):,}")
        c2.metric("إجمالي المكالمات", f"{int(total):,}")
        c3.metric("المكالمات الناجحة", f"{int(success):,}", delta=f"غير ناجحة {fail:,}", delta_color="off")
        c4.metric("نسبة النجاح", f"{float(success_rate):.1f}%")
        c5.metric("الوقت المهدر", _format_wasted_display(wasted_minutes))
...
def _render_activity_daily_chart(work, time_col, class_col=None):
    ...
    trend["اليوم"] = trend["_activity_time"].dt.strftime("%m/%d")
    trend["_day_key"] = trend["_activity_time"].dt.strftime("%Y-%m-%d")
    trend["_success_for_day"] = _activity_success_mask(trend, class_col)

    daily = trend.groupby(["_day_key", "اليوم", "_agent_display"], as_index=False).agg(
        **{"عدد المكالمات": ("_agent_display", "size"), "المكالمات الناجحة": ("_success_for_day", "sum")}
    )
    daily_totals = daily.groupby(["_day_key", "اليوم"], as_index=False).agg(
    ...
    sort_label = st.selectbox(
        "ترتيب الأيام",
        ["التاريخ تصاعديًا", "إجمالي المكالمات", "نسبة النجاح"],
        key="activity_daily_sort_v2",
        label_visibility="collapsed",
    )
    if sort_label == "إجمالي المكالمات":
        ordered = daily_totals.sort_values(["إجمالي المكالمات", "_day_key"], ascending=[False, True])
    elif sort_label == "نسبة النجاح":
        ordered = daily_totals.sort_values(["نسبة النجاح (%)", "إجمالي المكالمات", "_day_key"], ascending=[False, False, True])
    else:
        ordered = daily_totals.sort_values("_day_key")
    ordered_keys = ordered["_day_key"].tolist()
    label_map = dict(zip(daily_totals["_day_key"], daily_totals["اليوم"]))
    ordered_labels = [label_map[k] for k in ordered_keys]

    daily["_day_key"] = pd.Categorical(daily["_day_key"], categories=ordered_keys, ordered=True)
    daily = daily.sort_values(["_day_key", "_agent_display"])
    daily_totals = ordered.copy()
    daily_totals["اليوم"] = daily_totals["_day_key"].map(label_map)

    fig = px.bar(
        daily,
        x="_day_key",
        y="عدد المكالمات",
        color="_agent_display",
        barmode="stack",
        custom_data=["_agent_display"],
        template=PLOTLY_TEMPLATE,
        labels={"_agent_display": "المحصّل", "_day_key": "اليوم"},
        color_discrete_sequence=ACTIVITY_AGENT_PALETTE,
    )
    fig.add_trace(go.Scatter(
        x=daily_totals["_day_key"].astype(str),
        y=daily_totals["نسبة النجاح (%)"],
        name="نسبة النجاح",
        mode="lines+markers",
        line={"color": COLOR_WARN, "width": 2.5},
        marker={"size": 8, "color": COLOR_WARN, "line": {"color": THEME["surface"], "width": 1.5}},
        yaxis="y2",
        hovertemplate="<b>%{x}</b><br>نسبة النجاح: %{y:.1f}%<extra></extra>",
    ))
    fig.update_layout(**_activity_layout(
        title="النشاط اليومي ونسبة النجاح",
        xaxis_title="",
        ...
        xaxis={
            "type": "category",
            "categoryorder": "array",
            "categoryarray": ordered_keys,
            "tickmode": "array",
            "tickvals": ordered_keys,
            "ticktext": ordered_labels,
            "tickangle": 0,
        },
        yaxis={"rangemode": "tozero", "title": "عدد المكالمات"},
        yaxis2={
            "title": "نجاح %",
            "overlaying": "y",
            "side": "right",
        ...
        margin=dict(t=56, b=88, l=56, r=52),
    ))
    _style_activity_bars(fig, horizontal=False)
    # keep scatter hover as set
    for tr in fig.data:
        if tr.type == "scatter":
            tr.hovertemplate = "<b>%{x}</b><br>نسبة النجاح: %{y:.1f}%<extra></extra>"
...
def _render_activity_hourly_chart(work, time_col):
    ...
    hourly = (
        trend.groupby(["الساعة", "_agent_display"], as_index=False)
        .size()
        .rename(columns={"size": "عدد المكالمات"})
    )
    fig = px.bar(
        hourly,
        x="الساعة",
        ...
        custom_data=["_agent_display"],
        template=PLOTLY_TEMPLATE,
        labels={"_agent_display": "المحصّل"},
        color_discrete_sequence=ACTIVITY_AGENT_PALETTE,
    )
    fig.update_layout(**_activity_layout(
        title="توزيع المكالمات على ساعات اليوم",
        xaxis_title="الساعة",
        yaxis_title="عدد المكالمات",
        bargap=0.12,
        xaxis={
...
def _render_activity_no_answer_chart(agent):
    ...
    fig.update_layout(**_activity_layout(
        title="حالات لا يرد / مغلق",
        xaxis_title="العدد",
        yaxis_title="",
        margin=dict(t=56, b=88, l=120, r=24),
        yaxis={"categoryorder": "total ascending"},
    ))
    _style_activity_bars(fig, horizontal=True)
    fig.update_traces(customdata=long["المحصّل"])
...
def _render_activity_success_rate_chart(agent):
    ...
    fig.update_layout(**_activity_layout(
        title="نسبة النجاح حسب المحصّل",
        xaxis_title="%",
        yaxis_title="",
        xaxis={"range": [0, 105]},
        coloraxis_showscale=False,
        showlegend=False,
        margin=dict(t=56, b=56, l=120, r=36),
    ))
    fig.update_traces(
        texttemplate="%{x:.0f}%",
        textposition="outside",
...
def render_activity_dashboard(df, class_col=None, sales_col=None, time_col=None, break_start=None, break_end=None):
    ...
    st.caption("اضغط على محصّل داخل أي رسم لتطبيق فلتر على اللوحة.")
    ...
    with wasted_col:
        with st.container(border=True):
            if WASTED_TIME_COL in work.columns and "إجمالي الوقت المهدر (دقيقة)" in agent.columns:
                ...
                fig.update_layout(**_activity_layout(
                    title="الوقت المهدر حسب المحصّل",
                    xaxis_title="دقيقة",
                    yaxis_title="",
                    coloraxis_showscale=False,
                    showlegend=False,
...
def build_dashboard_html(df, class_col, sales_col, time_col, source_name="", filter_hint="", filter_summary=None) -> str:
    ...
    # ألوان هادئة موحّدة مع تويب النشاط
    background = ACTIVITY_EXPORT["bg"]
    surface = ACTIVITY_EXPORT["surface"]
    border = ACTIVITY_EXPORT["border"]
    text = ACTIVITY_EXPORT["text"]
    text_dim = ACTIVITY_EXPORT["text_dim"]
    export_success = ACTIVITY_EXPORT["success"]
    export_fail = ACTIVITY_EXPORT["fail"]
    export_accent = ACTIVITY_EXPORT["accent"]
    export_warn = ACTIVITY_EXPORT["warn"]
    ...
    def metric_card(card_id, label, value, color):
        return (
            f'<div id="{card_id}" class="kpi-card">'
            f'<div class="kpi-label">{label}</div>'
            f'<div data-role="value" class="kpi-value" style="color:{color}">{value}</div></div>'
        )

    css = (
        "<style>"
        "@import url('https://fonts.googleapis.com/css2?family=Tajawal:wght@400;500;700&display=swap');"
        ":root{--bg:" + background + ";--surface:" + surface + ";--border:" + border +
        ";--text:" + text + ";--muted:" + text_dim + ";--accent:" + export_accent +
        ";--success:" + export_success + ";--fail:" + export_fail + ";}"
        "*{box-sizing:border-box}"
        "body{margin:0;background:var(--bg);color:var(--text);font-family:'Tajawal',Tahoma,Arial,sans-serif;"
        "font-size:14px;line-height:1.65}"
        "main{max-width:1280px;margin:0 auto;padding:28px 22px 40px}"
        ".panel{background:var(--surface);border:1px solid var(--border);border-radius:16px;padding:20px 22px;margin-bottom:16px}"
        ".eyebrow{font-size:11px;letter-spacing:.12em;color:var(--accent);font-weight:500;margin-bottom:4px}"
        "h1{margin:0 0 6px;font-size:24px;font-weight:700;color:var(--text)}"
        "h2{margin:0 0 12px;font-size:15px;font-weight:600;color:var(--text);text-align:right}"
        ...
        "padding:18px 12px;text-align:center;min-height:96px}"
        ".kpi-label{color:var(--muted);font-size:12px;margin-bottom:10px;font-weight:500}"
        ".kpi-value{font-size:22px;font-weight:700;line-height:1.25}"
        ".charts{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;margin-bottom:16px}"
        ".chart-card{background:var(--surface);border:1px solid var(--border);border-radius:16px;padding:10px 12px 6px;min-width:0}"
        "table{width:100%;border-collapse:collapse;font-size:13px}"
        "th{padding:10px 8px;border-bottom:1px solid var(--border);color:var(--muted);font-weight:600;text-align:right;white-space:nowrap}"
        "td{padding:9px 8px;border-bottom:1px solid rgba(148,163,184,.15);white-space:nowrap;color:var(--text)}"
        "tr:hover td{background:rgba(79,140,141,.04)}"
        "footer{color:var(--muted);font-size:12px;text-align:center;margin-top:20px}"
        "#filter-status{text-align:center;color:var(--muted);font-size:12px;margin-top:10px}"
        ".filters-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;align-items:end}"
        ".multi-trigger,select,input[type=date],button{font-family:inherit;font-size:13px}"
        "</style>"
    )
    parts = [
        ...
        f'<div class="subtitle">مصدر البيانات: {escape(source_name or "ملف النشاط")}</div>',
    ]
    if filter_hint:
    ...
    parts.extend([
        '</header>',
        '<section class="panel">',
        f'<h2 style="margin:0 0 10px;text-align:right;font-size:15px;color:{text}">فلاتر التقرير</h2>',
    ...
    parts.extend([
        ...
        '<section id="kpi-grid">',
        metric_card("kpi-agents", "عدد المحصّلين", f"{agent_count:,}", text),
        ...
        metric_card("kpi-wasted", "الوقت المهدر", _format_wasted_display(wasted), export_warn),
        '</section>',
    ])
    ...
    if time_col and time_col in work.columns:
        ...
        if not timed.empty:
            ...
            day_fig = px.bar(daily, x="اليوم", y="عدد المكالمات", color="_agent_display", barmode="stack", custom_data=["_agent_display"], template=export_template, labels={"_agent_display":"المحصّل"}, color_discrete_sequence=ACTIVITY_AGENT_PALETTE)
            day_fig.add_trace(go.Scatter(x=daily_totals["اليوم"].astype(str), y=daily_totals["نسبة النجاح (%)"], name="نسبة النجاح", mode="lines+markers", line={"color": export_warn, "width": 2.5}, marker={"color": export_warn, "size": 7}, yaxis="y2", hovertemplate="<b>%{x}</b><br>نسبة النجاح: %{y:.1f}%<extra></extra>"))
            ...
            figs.append(("النشاط اليومي ونسبة النجاح", day_fig))
            ...
            hour_fig = px.bar(hourly, x="الساعة", y="عدد المكالمات", color="_agent_display", barmode="stack", custom_data=["_agent_display"], template=export_template, labels={"_agent_display":"المحصّل"}, color_discrete_sequence=ACTIVITY_AGENT_PALETTE)
            ...
            figs.append(("توزيع المكالمات على ساعات اليوم", hour_fig))
    ...
    if sub_col:
        ...
        no_fig = px.bar(state_long, x="العدد", y="المحصّل", orientation="h", color="الحالة", barmode="stack", template=export_template, category_orders={"الحالة":ACTIVITY_NO_ANSWER_STATES}, color_discrete_sequence=ACTIVITY_STATE_PALETTE)
        ...
        figs.append(("حالات لا يرد / مغلق", no_fig))

    parts.append('<section class="charts">')
    include_js = True
    for index, (heading, fig) in enumerate(figs):
        fig.update_layout(
            title=None,  # العنوان في h2 بالـ HTML
            font=dict(family="Tajawal, Tahoma, sans-serif", size=12, color=text_dim),
            paper_bgcolor="rgba(0,0,0,0)",
            plot_bgcolor="rgba(0,0,0,0)",
            margin=dict(t=20, b=70, l=48, r=24),
            height=340,
            legend=dict(orientation="h", y=-0.18, x=0.5, xanchor="center", font=dict(size=11)),
        )
        parts.append(f'<article id="chart-card-{index}" class="chart-card"><h2>{heading}</h2>')
        parts.append(pio.to_html(fig, full_html=False, include_plotlyjs=include_js, config=PLOTLY_CONFIG, div_id=f"activity_plot_{index}", default_width="100%", default_height="340px"))
    ...
    if sales_col and sales_col in df.columns:
        ...
        parts.append('<section id="activity-summary-table" class="panel"><h2>ملخص أداء كل محصّل</h2><div style="overflow-x:auto"><table><thead><tr>')
...
""".replace('__ACTIVITY_DATA__', raw_records_json).replace('__AGENT_COLORS__', agent_color_json).replace('__STATE_COLORS__', state_color_json).replace('__ACCENT__', json.dumps(export_accent)).replace('__SUCCESS__', json.dumps(export_success)).replace('__FAIL__', json.dumps(export_fail)).replace('__DATE_MIN__', export_date_min).replace('__DATE_MAX__', export_date_max)
    ...
    parts.extend(['<footer>تم إنشاء التقرير من لوحة تحليل نشاط المحصّلين</footer></main></body></html>'])
    return "".join(parts)
    ...
    with st.popover(trigger_text, use_container_width=True):
        ...
        with action_clear:
            if st.button("إلغاء الكل", key=f"{state_key}__clear_all", use_container_width=True):
                ...
                st.rerun()
        st.caption("يمكن اختيار أكثر من قيمة.")
...
def _render_slicers(df, sales_col, time_col):
    ...
    with st.container(border=True):
        ...
        slicer_cols = st.columns(4, gap="medium")
        with slicer_cols[0]:
            selected_agents = _render_native_multi_slicer(
                "المحصّلون", agents, "dash_agent_slicer_v6", "كل المحصّلين"
            )
        with slicer_cols[1]:
        ...
        st.caption("اترك الفلاتر على «الكل» لعرض كل البيانات. اضغط على أي محصّل داخل الرسوم لتطبيق فلتر سريع.")
