...
- قسم "النشاط" فيه: رفع ملف -> حذف أول صف بعد الهيدر -> اختيار عمود نص المكالمة
  -> زرار "ابدأ التصنيف" (بينادي موديل Hugging Face، ياخد ناجحة/غير ناجحة، ويحولها لـ 1/0)
  -> تشارتات -> تحميل الملف.
  المنطق الفعلي لنداء الموديل موجود في classify_call_with_model() — لو الموديل شغال بطريقة
  مختلفة عن Hugging Face Inference API، عدّل جوه الدالة دي بس.
...
def detect_text_column(df: pd.DataFrame):
    """بيحاول يلاقي عمود نص المكالمة/الملاحظة تلقائيًا (اللي هيتبعت للموديل)."""
    keywords = [
        "مكالمة", "المكالمة", "نص", "النص", "ملاحظة", "الملاحظة",
        "تفريغ", "التفريغ", "transcript", "text", "Text",
    ]
...
def classify_call_with_model(text: str, model_id: str, hf_token: str = "") -> str:
    """بيبعت نص المكالمة لموديل التصنيف (Hugging Face Inference API)
    ويرجع 'ناجحة' أو 'غير ناجحة'.

    ملحوظة مهمة: الدالة دي مبنية افتراضيًا على إن الموديل بيتنادى عن طريق
    Hugging Face Inference API (huggingface_hub). لو الموديل بتاعك شغال بطريقة
    تانية (مثلاً محمّل محليًا بـ transformers، أو عندك API مختلف)، غيّر جوه
    الدالة دي بس — الباقي (الزرار، التحويل لـ 1/0، التشارتات) هيفضل شغال زي ما هو.
    """
    try:
        from huggingface_hub import InferenceClient
    except ImportError:
        st.error(
            "المكتبة huggingface_hub مش متثبتة. ثبّتها بـ: "
    ...
    try:
        ...
        # لو نداء الموديل فشل لسبب ما (نص فاضي، مشكلة شبكة، الخ)
        # بنعتبرها غير ناجحة افتراضيًا — عدّل السلوك ده لو محتاج حاجة تانية.
        return FAIL_LABEL

    if not result:
        return FAIL_LABEL

    top = max(result, key=lambda r: r.get("score", 0)) if isinstance(result, list) else result
    label = str(top.get("label", "")).strip()

    # الموديل ممكن يرجّع "ناجحة" مباشرة، أو LABEL_1/POSITIVE حسب إعداده
    if "ناجح" in label and "غير" not in label:
        return SUCCESS_LABEL
    if label.upper() in ("1", "LABEL_1", "POSITIVE"):
        return SUCCESS_LABEL
    return FAIL_LABEL


def classify_dataframe_with_model(
    df: pd.DataFrame, text_col: str, model_id: str, hf_token: str = ""
) -> pd.DataFrame:
    """بيمرّ على كل صف، يبعت نص المكالمة للموديل، ياخد التصنيف (ناجحة/غير ناجحة)
    ويضيف عمود 'الحالة' (نصي) وعمود 'التصنيف' (رقمي: ناجحة -> 1، غير ناجحة -> 0)."""
    out = df.copy()
    labels = []

    total = len(out)
    progress = st.progress(0, text="⏳ جاري تصنيف المكالمات بالموديل...")
    for i, text in enumerate(out[text_col].astype(str).tolist()):
        labels.append(classify_call_with_model(text, model_id, hf_token))
        if total:
            progress.progress((i + 1) / total, text=f"⏳ جاري تصنيف المكالمات... ({i + 1}/{total})")
    progress.empty()

    out["الحالة"] = labels
    out["التصنيف"] = out["الحالة"].map({SUCCESS_LABEL: 1, FAIL_LABEL: 0})
    return out
...
def render_nashat():
    ...
    if uploaded_file is not None:
        ...
        st.markdown('<div class="section-title">🎯 التصنيف بالموديل</div>', unsafe_allow_html=True)

        with st.expander("⚙️ إعدادات الموديل", expanded=False):
            model_id = st.text_input(
                "اسم/مسار الموديل على Hugging Face",
                value=st.secrets.get("MODEL_ID", "") if hasattr(st, "secrets") else "",
                placeholder="مثال: username/call-classifier-model",
                key="nashat_model_id",
            )
            hf_token = st.text_input(
                "Hugging Face Token (لو الموديل خاص)",
                value=st.secrets.get("HF_TOKEN", "") if hasattr(st, "secrets") else "",
                type="password",
                key="nashat_hf_token",
            )

        default_text_col = detect_text_column(df)
        default_index = list(df.columns).index(default_text_col) if default_text_col in df.columns else 0
        text_col = st.selectbox(
            "اختار عمود نص المكالمة (اللي هيتبعت للموديل)",
            list(df.columns),
            index=default_index,
            key="nashat_text_col_select",
        )

        if st.button("🚀 ابدأ التصنيف", key="nashat_classify_btn"):
            if not model_id:
                st.error("لازم تكتب اسم/مسار الموديل الأول من إعدادات الموديل فوق ⚙️")
            else:
                with st.spinner("جاري تشغيل الموديل على المكالمات..."):
                    classified = classify_dataframe_with_model(df, text_col, model_id, hf_token)
                st.session_state["nashat_classified_df"] = classified
                st.session_state["nashat_text_col"] = text_col
